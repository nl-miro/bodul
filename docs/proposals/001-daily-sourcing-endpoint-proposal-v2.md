...
The endpoint returns `200 OK` with a JSON body:
...
The handler return type is `poem::Result<poem::web::Json<io::DailySourcingResponse>>`.
`poem::web::Json<T>` returns `200 OK` by default — no tuple wrapper is needed.
...
`500 Internal Server Error`. UUID generation via `Uuid::new_v4()` is treated
...
3. **Correct token → 200 OK with valid response schema.**
...
4. **Enqueue-only: handler does not execute the command.**
   - Assert that after the 200 response, no `StartDailySourcing` handler has
...
- `daily-sourcing.bru` — happy path, expects `200 OK` with JSON response
