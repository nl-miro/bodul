...
mod domain {
    ...
    pub struct EntryRecord {
        ...
        pub queue_delay_seconds: i64,
        pub time_in_system_seconds: i64,
        pub processing_latency_seconds: Option<i64>,
    }
    ...
    impl EntryRecord {
        ...
        pub fn attempt_phase(&self) -> &'static str {
            match self.attempts {
                i32::MIN..=0 => "queued",
                1 => "initial",
                _ => "retry",
        ...
        pub fn extra_info_errors(&self) -> Vec<String> {
            let Some(extra_info) = &self.extra_info else {
                return vec![];
            };

            match extra_info.get("errors") {
        ...
        fn entry(
            ...
            (
                ...
                EntryRecord {
                    ...
                    time_in_system_seconds: 0,
                    processing_latency_seconds: Some(0),
                },
            )
        }
...
mod application {
    ...
    struct RawEntryRow {
        ...
        #[diesel(sql_type = BigInt)]
        time_in_system_seconds: i64,
        #[diesel(sql_type = Nullable<BigInt>)]
        processing_latency_seconds: Option<i64>,
    }
        ...
        let rows = sql_query(
            ...
            FROM (
                SELECT
                    ...
                    GREATEST(0, EXTRACT(EPOCH FROM (scheduled_at - received_at)))::bigint
                        AS queue_delay_seconds,
                    GREATEST(0, EXTRACT(EPOCH FROM (updated_at - received_at)))::bigint
                        AS time_in_system_seconds,
                    CASE
                        WHEN processed_at IS NULL THEN NULL::bigint
                        ELSE GREATEST(
                            0,
                            EXTRACT(EPOCH FROM (processed_at - received_at))
                        )::bigint
                    END AS processing_latency_seconds
                FROM inbox_entries
        ...
        rows.into_iter()
            .map(|row| {
                ...
                let extra_info = row
                    ...
                    .map_err(|err| {
                        format!(
                            "invalid extra_info json for {} {}: {err}",
                            key.kind.as_str(),
                            key.id
                        )
                    })?;
                ...
                Ok((
                    ...
                    EntryRecord {
                        ...
                        time_in_system_seconds: row.time_in_system_seconds,
                        processing_latency_seconds: row.processing_latency_seconds,
                    },
                ))
            })
            .collect()
    ...
    fn render_html(
        ...
        html.push_str(
            "body{font-family:system-ui,-apple-system,BlinkMacSystemFont,\"Segoe UI\",sans-serif;background:#111827;color:#e5e7eb;margin:0;padding:2rem;line-height:1.5;}\
             ...
             .error-alert{margin:0.75rem 0;background:#7f1d1d;border:2px solid #ef4444;border-radius:10px;padding:0.75rem 0.9rem;color:#fee2e2;box-shadow:0 0 0 1px rgba(239,68,68,0.25) inset;}\
             .error-alert strong{display:block;margin-bottom:0.35rem;color:#fecaca;text-transform:uppercase;letter-spacing:0.04em;font-size:0.82rem;}\
             .error-alert pre{margin:0;white-space:pre-wrap;word-break:break-word;font-weight:600;}",
        );
    ...
    fn render_trace_node(
        ...
        render_extra_info_error_alert(html, entry);
        render_processing_details(html, entry);
    ...
    fn render_processing_details(html: &mut String, entry: &EntryRecord) {
        let mut details = vec![
            ("attempts", entry.attempts.to_string()),
            ("attempt_phase", entry.attempt_phase().to_string()),
            ("retry_count", entry.retry_count().to_string()),
            (
                "scheduled_delay",
                format_duration(entry.queue_delay_seconds),
            ),
            (
                "time_in_system",
                format_duration(entry.time_in_system_seconds),
            ),
        ];

        if let Some(processing_latency_seconds) = entry.processing_latency_seconds {
            details.push((
                "processing_latency",
                format_duration(processing_latency_seconds),
            ));
        }

        render_metadata_table(html, &details);
    }

    fn render_extra_info_error_alert(html: &mut String, entry: &EntryRecord) {
        let errors = entry.extra_info_errors();
        if errors.is_empty() {
            return;
        }
        ...
        let _ = write!(
            html,
            "<div class=\"error-alert\"><strong>Extra info errors</strong><pre>{}</pre></div>",
    ...
    fn render_json_details(html: &mut String, label: &str, raw: &str, is_open: bool) {
        ...
        render_text_details(html, label, &formatted, is_open);
    }
    ...
    fn render_text_details(html: &mut String, label: &str, text: &str, is_open: bool) {
    ...
    fn format_duration(total_seconds: i64) -> String {
        let mut seconds = total_seconds.max(0);
